...
const float PALM_STEADY_DEG = 0.03f;  // allow ±0.1g around the avg
...
const float LIT_MIN_FALL = 0.8f;    // little finger floor for fall
...
void taskSensor(void *pv) {
  ...
  while (true) {
    ...
    mpuLITT.readAccel();
    tmp.palmMag = sqrt(
      sq(mpuPALM.ax_g) + sq(mpuPALM.ay_g) + sq(mpuPALM.az_g));
...
void setDirectionThresholds(MPU_6050 &mpu){
  FRONT_TH = (mpu.getMinAY() + mpu.getNeutralAY()) / 2;
  BACK_TH  = (mpu.getMaxAY() + mpu.getNeutralAY()) / 2;
  LEFT_TH  = (mpu.getMinAX() + mpu.getNeutralAX()) / 2;
  RIGHT_TH = (mpu.getMaxAX() + mpu.getNeutralAX()) / 2;
}
